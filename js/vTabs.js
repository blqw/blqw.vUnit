...
(function(window) {
  ...
  var template = '<ul style="padding-left: 0; margin-bottom: 0; list-style: none; border-bottom: 1px solid #ddd;" @mouseout="$data._hover=-1">\
        ...
        @mouseover="$data._hover=index" >{{tabValue(tab,"text","")}}</a>\
    </li>\
    </ul>';
  ...
  var vTabs = vue.extend({
    data: function() {
      ...
      if (props && props.hasOwnProperty("tabs")) {
        delete data.tabs;
      }
      if (props && props.hasOwnProperty("text")) {
        data.tabFields.text = props.text;
      }
      if (props && props.hasOwnProperty("el")) {
        data.tabFields.el = props.el;
      }
      if (props && props.hasOwnProperty("id")) {
        data.tabFields.id = props.id;
      }
      if (props && props.hasOwnProperty("disabled")) {
        data.tabFields.disabled = props.disabled;
      }
      return data;
    },
    props:["tabs","text","el","id","disabled"],
    computed: {
      selectedTab: {
        get: function() {
      ...
      selectedId: {
        get: function() {
          this.tabValue(selectedTab, "id", null);
        },
        set: function(id) {
          this.selectedIndex = this.getById(id);
        }
      }
    },
    methods: {
      ...
      formatter: function(tab) {
        return this.tabValue(tab, "text", "");
      },
      ...
      onChanged: function(oldSelectedIndex) {
        this.$emit("changed", {
          vm: this,
          oldSelectedIndex
        });
